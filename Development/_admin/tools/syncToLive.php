...
if( $server!='' && $fromclient!='' && $toclient!='' ){
	...
	switch( $cmd ){
		case "":
			$modules = SyncDatabase::getModules($localDatabase);
		...
		case "syncronise":
			...
			foreach( $modules as $key=>$value ){
				if( $__APPLICATION_['url']->getRequestVar('smodule'.$value['sys_module_id'])!='' ){
					SyncDatabase::syncModule($localDatabase,$remoteDatabase,$value);					
				}
				if( $__APPLICATION_['url']->getRequestVar('cmodule'.$value['sys_module_id'])!='' ){
					SyncDatabase::syncModuleContent($localDatabase,$remoteDatabase,$value['tableName']);					
				}
			}
			
			if( $__APPLICATION_['url']->getRequestVar('systemtables')!='' ){
				SyncDatabase::syncSystemTables($localDatabase,$remoteDatabase);
			}
		break;		
	}
