					...
					foreach ( $xml->chunk [0]->line as $line ) {
						if (count ( $line->val ) > 0) {
							...
							foreach ( $line->val as $val ) {
								...
								if ($dimName == 'ATTYPDESC') {
									$user = $db->fetchrownamed ( 'select * from mod_debtorTransactionType where name="' . $val . '"' );
									$lineVars [$dimName] = $user ['mod_debtorTransactionType_id'];
								}
							...
							if ($tableToSync == 'debtorComment') {
								...
								if (! ($db->fetchrownamed ( 'select * from mod_debtorTransaction where name="' . $value ['name'] . ' - ' . $lineVars ['ATDAT'] . '-' . $lineVars ['ATREF'] . '-R' . $lineVars ['ATAMT'] . '-'.$lineVars['ATSEQ'].'"' ))) {
									$query = 'insert into mod_debtorTransaction( name, actualDate, transactionType, reference, amount, dateCaptured, narrative, debtor ) values( 
									"' . $value ['name'] . ' - ' . $lineVars ['ATDAT'] . '-' . $lineVars ['ATREF'] . '-R' . $lineVars ['ATAMT'] . '-'.$lineVars['ATSEQ']. '","' . $lineVars ['ATDATDB'] . '",' . $lineVars ['ATTYPDESC'] . ',"' . $lineVars ['ATREF'] . '","' . $lineVars ['ATAMT'] . '","' . $lineVars ['ATMDATDB'] . '","' . $lineVars ['ATNAR'] . '",' . $value ['mod_debtor_id'] . ')';
									$db->__runquery_ ( $query );
									echo $query;
								}
